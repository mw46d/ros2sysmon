...
class DisplayManager:
    ...
    def _create_layout(self) -> Layout:
        ...
        layout.split_column(
            ...
            Layout(name="help", size=3),  # Help panel with border
        )
    ...
    def _initialize_panels(self, layout: Layout):
        layout["header"].update(Panel("x", title="System Resources"))
        layout["left"].update(Panel("Left pane disabled", title="Left"))
        layout["middle"].update(Panel("Middle pane disabled", title="Middle"))
        layout["right"].update(Panel("Right pane disabled", title="Right"))
        layout["alerts"].update(Panel("y", title="System Alerts"))
        layout["help"].update(
            Panel("Commands: Press 'x' or Ctrl+C to quit", title="Help")
        )
    ...
    def _update_left_panel(self, nodes):
        ...
        #     )
        # else:
        #     assert("invalid display_mode")

        self.layout["left"].update(Panel("Left pane disabled", title="Left"))

    def _update_middle_panel(self, metrics):
        # if self.display_mode == 'ros':
        #     # ROS Mode: TF Frames (coming soon)
        ...
        self.layout["middle"].update(Panel("Middle pane disabled", title="Middle"))

    def _update_right_panel(self, topics, metrics):
        """Update the right panel based on display mode."""
        # if self.display_mode == 'ros':
        #     # ROS Mode: Topics panel
        #     self.layout["right"].update(
        #         self._create_topics_panel(topics)
        #     )
        # elif self.display_mode == 'system':
        #     # System Mode: Network Stats
        #     self.layout["right"].update(
        ...
        self.layout["right"].update(Panel("Right pane disabled", title="Right"))
    ...
    def _update_display(self, shared_data: SharedDataStore):
        ...
        try:
            ...
            self.layout["header"].update(Panel(f"Error: {e}", title="Display Error"))
    ...
    def _create_system_overview(self, metrics: Optional[SystemMetrics]) -> Panel:
        ...
        # Right column data
        right_data = [f"Load: {metrics.load_average[0]:.2f}  Uptime: {metrics.uptime}"]
    ...
    def _create_alerts_panel(self, alerts: List[SystemAlert]) -> Panel:
        ...
        for alert in recent_alerts:
            style = {"ERROR": "red", "WARN": "yellow", "INFO": "blue"}.get(
                alert.level, "white"
            )
        ...
        border_style = (
            "red" if any(a.level == "ERROR" for a in recent_alerts) else "yellow"
        )
    ...
    def _create_nodes_panel(self, nodes: List[ROSNodeInfo]) -> Panel:
        ...
        if not nodes:
            return Panel("No ROS nodes detected", title="ROS2 Nodes")
        ...
        for node in sorted_nodes[:10]:  # Show top 10 nodes
            ...
            # Color code status
            status_color = {"OK": "green", "WARN": "yellow", "ERROR": "red"}.get(
                node.status, "white"
            )
    ...
    def _create_topics_panel(self, topics: List[TopicMetrics]) -> Panel:
        ...
        if not topics:
            return Panel("No ROS topics detected", title="ROS2 Topics")
    ...
    def _create_processes_panel(self) -> Text:
        ...
        try:
            ...
            for proc in top_processes:
                ...
                cpu_color = (
                    "red" if cpu_pct > 50 else "yellow" if cpu_pct > 10 else "green"
                )
            ...
            return Text(
                "ROS Processes - Error loading ROS processes", style="bold white on red"
            )
        ...
        return Text(full_content, style="white on dark_magenta")
    ...
    def _create_system_details_panel(self, metrics: Optional[SystemMetrics]) -> Text:
        ...
        try:
            ...
            content.append(
                f"Used: {disk.used / (1024**3):.1f} GB ({metrics.disk_percent:.1f}%)"
            )
        ...
        return Text(full_content, style="white on dark_cyan")
    ...
    def _create_network_stats_panel(self, metrics: Optional[SystemMetrics]) -> Text:
        ...
        if metrics and metrics.network_latency is not None:
            ...
            content.append(
                f"Ping to 8.8.8.8: [{latency_color}]{metrics.network_latency:.1f} ms[/{latency_color}]"
            )
        else:
        ...
        try:
            ...
            for interface, stats in list(net_stats.items())[
                :5
        ...
        if metrics:
            ...
            if metrics.temperature:
                ...
                content.append(
                    f"Temperature: [{temp_color}]{metrics.temperature:.1f}°C[/{temp_color}]"
                )
            if metrics.battery_percent:
                batt_color = (
                    "red"
                ...
                content.append(
                    f"Battery: [{batt_color}]{metrics.battery_percent:.1f}%[/{batt_color}]"
                )
        ...
        return Text(full_content, style="white on dark_yellow")

    def _create_help_panel(self) -> Panel:
        """Create help panel with keyboard shortcuts."""
        help_text = "Commands: Press 'x' or Ctrl+C to quit"
        return Panel(help_text, title="Help")
